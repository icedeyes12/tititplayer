...
from tititplayer.tui.client import APIClient, APIClientError, get_client, close_client
...
class TestAPIClient:
    ...
    async def test_play(self, api_client: APIClient):
        ...
        with patch.object(api_client, "_request", new_callable=AsyncMock) as mock_request:
            ...
            mock_request.assert_called_once_with("POST", "/api/v1/playback/play", json={"track_id": 1})
    ...
    async def test_toggle_pause(self, api_client: APIClient):
        ...
        with patch.object(api_client, "_request", new_callable=AsyncMock) as mock_request:
            ...
            await api_client.connect()
            result = await api_client.toggle_pause()
    ...
    async def test_set_volume(self, api_client: APIClient):
        ...
        with patch.object(api_client, "_request", new_callable=AsyncMock) as mock_request:
            ...
            await api_client.connect()
            result = await api_client.set_volume(50)
            
            mock_request.assert_called_once_with("POST", "/api/v1/playback/volume", json={"volume": 50})
            await api_client.disconnect()
    ...
    async def test_next_track(self, api_client: APIClient):
        ...
        with patch.object(api_client, "_request", new_callable=AsyncMock) as mock_request:
            ...
            await api_client.connect()
            result = await api_client.next_track()
    ...
    async def test_add_to_queue(self, api_client: APIClient):
        ...
        with patch.object(api_client, "_request", new_callable=AsyncMock) as mock_request:
            ...
            await api_client.connect()
            result = await api_client.add_to_queue([1, 2, 3], position=0)
    ...
    async def test_toggle_shuffle(self, api_client: APIClient):
        ...
        with patch.object(api_client, "_request", new_callable=AsyncMock) as mock_request:
            ...
            await api_client.connect()
            result = await api_client.toggle_shuffle()
